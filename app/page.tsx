...
export default function Home() {
  ...
  <Link href="/dashboard" style={{
    ...
    Dashboard
      ...
      <style jsx>{`
        .qai-main { min-height:100vh; background:#0a0e1a; color:#e8eaf0; font-family:'DM Sans','Segoe UI',sans-serif; padding:0 0 80px; }
        .qai-header { display:flex; align-items:center; gap:16px; padding:28px 48px; border-bottom:1px solid rgba(255,255,255,0.06); }
        .qai-logo { font-size:28px; font-weight:700; letter-spacing:-1px; }
        .logo-q { color:#4a9eff; }
        .logo-ai { color:#e8eaf0; }
        .qai-tagline { font-size:12px; letter-spacing:0.12em; text-transform:uppercase; color:rgba(255,255,255,0.35); padding-left:16px; border-left:1px solid rgba(255,255,255,0.12); }
        .qai-intro { max-width:680px; margin:56px auto 48px; padding:0 24px; text-align:center; }
        .qai-intro h1 { font-size:32px; font-weight:600; letter-spacing:-0.5px; margin:0 0 14px; color:#f0f2f8; }
        .qai-intro p { font-size:15px; line-height:1.7; color:rgba(255,255,255,0.45); margin:0; }
        .inputs-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; max-width:1100px; margin:0 auto; padding:0 24px; }
        @media(max-width:900px){ .inputs-grid{grid-template-columns:1fr;} .qai-header{padding:20px 24px;} }
        .qai-action { max-width:1100px; margin:32px auto 0; padding:0 24px; display:flex; flex-direction:column; align-items:center; gap:12px; }
        .run-btn { width:100%; max-width:480px; padding:18px 32px; border-radius:12px; border:none; font-size:16px; font-weight:600; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:10px; transition:all 0.2s; }
        .run-btn.waiting { background:rgba(255,255,255,0.06); color:rgba(255,255,255,0.3); cursor:not-allowed; border:1px solid rgba(255,255,255,0.08); }
        .run-btn.ready { background:linear-gradient(135deg,#4a9eff,#3a7de0); color:white; box-shadow:0 4px 24px rgba(74,158,255,0.35); }
        .run-btn.ready:hover { transform:translateY(-1px); box-shadow:0 8px 32px rgba(74,158,255,0.45); }
...
function DropZone({ label, title, description, accept, state, inputRef, onDrop, onRemove, onChange, color, icon }: DropZoneProps) {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" multiple accept={accept} style={{ display:'none' }} onChange={onChange} />
        ...
        <div className="file-list">
          {state.files.map((f, i) => (
            <div key={i} className="file-item">
              ...
              <button className="file-remove" onClick={e => { e.stopPropagation(); onRemove(i) }}>×</button>
            </div>
          ))}
          <button className="add-more" onClick={e => { e.stopPropagation(); inputRef.current?.click() }}>+ Add more files</button>
        </div>
      )}
      <style jsx>{`
        .drop-zone { background:rgba(255,255,255,0.03); border:1.5px dashed rgba(255,255,255,0.1); border-radius:16px; padding:28px 24px; cursor:pointer; transition:all 0.2s; display:flex; flex-direction:column; gap:10px; min-height:320px; }
        .drop-zone:hover, .drop-zone.drag-over { background:${c.bg}; border-color:${c.border}; border-style:solid; }
        .drop-zone.has-files { background:${c.bg}; border-color:${c.border}; border-style:solid; cursor:default; }
        .zone-label { font-size:10px; font-weight:700; letter-spacing:0.14em; text-transform:uppercase; color:${c.accent}; }
        .zone-icon { font-size:28px; line-height:1; }
        .zone-title { font-size:16px; font-weight:600; color:#f0f2f8; letter-spacing:-0.2px; }
        .zone-desc { font-size:12px; line-height:1.6; color:rgba(255,255,255,0.38); }
        .zone-cta { margin-top:auto; display:flex; flex-direction:column; gap:6px; }
        .zone-cta span { font-size:13px; color:rgba(255,255,255,0.4); }
        .zone-accept { font-size:10px; color:rgba(255,255,255,0.2); font-family:monospace; letter-spacing:0.05em; }
        .file-list { display:flex; flex-direction:column; gap:6px; margin-top:8px; }
        .file-item { display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.05); border-radius:8px; padding:8px 10px; font-size:12px; }
        .file-name { flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#e8eaf0; }
        .file-size { color:rgba(255,255,255,0.3); white-space:nowrap; }
        .file-remove { background:none; border:none; color:rgba(255,255,255,0.3); cursor:pointer; font-size:16px; line-height:1; padding:0 2px; transition:color 0.15s; }
        .file-remove:hover { color:#ff8080; }
        .add-more { background:none; border:1px dashed rgba(255,255,255,0.15); border-radius:8px; color:rgba(255,255,255,0.35); font-size:12px; padding:7px; cursor:pointer; transition:all 0.15s; text-align:center; }
        .add-more:hover { border-color:${c.border}; color:${c.accent}; }
      `}</style>
    </div>
  )
}